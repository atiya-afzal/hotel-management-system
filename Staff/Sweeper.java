...
public class Sweeper extends StaffFunctions implements Tools, MethodFormer {
...
//    ui components
    ...
    public void showDataForOne() {
        ...
        roomNumberDialog.setResultConverter(dialogButton -> {
            if (dialogButton == checkButton) {
                try {
                    ...
                    showError( "Please enter a valid integer for the room number.");
                    return null;
                }
            }
            return null;
        ...
        result.ifPresent(roomNumber -> {
            ...
            if (!roomFound) {
                showError( "There is no room with number " + roomNumber + ".");
            }
        });
    }
    ...
    public void updateStatus() {
        ...
        roomNumberField.textProperty().addListener((observable, oldValue, newValue) -> {
            try {
                ...
                if (!found) {
                    currentStatusLabel.setText("Room Status: Not Found");
                }
            } catch (NumberFormatException e) {
                currentStatusLabel.setText("    Invalid Input");
            }
        });
