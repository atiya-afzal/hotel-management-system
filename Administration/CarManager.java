...
public class CarManager implements Regex, Tools , MethodFormer {
        ...
        addButton.addEventFilter(ActionEvent.ACTION, event -> {
            try {
                ...
                if (!plate.matches(LPlateNumber)) {
                    showError("License plate number must have 3 letters, a space, and 1 to 4 digits.");
                    event.consume(); // Prevent dialog from closing
                    return;//as in room
                }
                    ...
                    for (Car existingCar : carlist) {
                        ...
                        if(existingCar.getL_plateno().equals(plate)){
                            event.consume(); // Prevent dialog from closing
                            showError("Car lisence plate number already exists.");
                            return;
                        }
        ...
        dialog.showAndWait().ifPresent(updatedCar -> {
            for (Car car : carlist) {
                ...
                if (car.getId() == updatedCar.getId()) {
                    boolean found=false;
                        if(!(updatedCar.getL_plateno().equals(car.getL_plateno()))){
                           for(Car c:carlist){
                               if(c.getL_plateno().equals(updatedCar.getL_plateno())){
                                   showError("car with this lisence palte number already exists");
