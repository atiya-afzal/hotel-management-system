...
//    ui components
    ...
    public void allCarsData() {
        ...
        TableColumn<Car, String> L_plateColumn = new TableColumn<>("License Plate No.");
        L_plateColumn.setCellValueFactory(new PropertyValueFactory<>("L_plateno"));


        tableView.getColumns().addAll(idColumn, modelColumn, statusColumn, L_plateColumn);
    ...
    public void exitToStaffPortal(ActionEvent event) {
        if (previous instanceof StaffPortal) {
            switchScene(event, "/com/example/Lodgify/StaffPortal.fxml", "Service Staff Portal", previous);
        }
